...
// 合約: XOIOV4 — Chainlink VRF 單雙遊戲(整批單一隨機數)
...
/**
 * @title XOIO v4 — Decentralized Betting Game (Chainlink VRF, 單一隨機數)
 * @notice 玩法:
 ...
 * @dev v4 變更:整批下注只請求 1 個隨機數(numWords=1),
 *      批次內所有注共用同一個開獎尾數,與 v3(每注獨立隨機數)不同。
 *      Owner 使用 Chainlink ConfirmedOwner(由 VRFConsumerBaseV2Plus 內建)
 */
contract XOIOV4 is VRFConsumerBaseV2Plus {
    // ============ 常數 ============
    ...
    uint32 public numWords = 1;   // 固定 1:整批共用一個隨機數
    ...
    function _placeBets(uint256[] memory amounts, uint8[] memory choices) internal {
        ...
        // 請求 Chainlink VRF(v4:整批只請求 1 個隨機數,所有注共用同一開獎)
        uint32 words = 1;
